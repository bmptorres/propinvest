const CACHE_NAME = 'propinvest-v20';
const RUNTIME_CACHE = 'propinvest-runtime-v20';
...
self.addEventListener('install', event => {
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      ...
      .then(() => {
        console.log('[SW v20] Installed successfully.');
        return self.skipWaiting();
      })
      .catch(error => {
        console.error('[SW v20] Install failed:', error);
        throw error;
      })
  );
});
...
self.addEventListener('activate', event => {
  ...
  event.waitUntil(
    caches.keys().then(keys =>
      ...
      console.log('[SW v20] Active. Claiming clients.');
